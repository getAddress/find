...
export function find(container_id, api_key = "", options = new SetupOptions()) {
    let client = new Client(api_key, options.endpoints?.autocomplete_url, options.endpoints?.get_url);
    ...
    if (context && client) {
        ...
        let find = new Find(context, client, inputConfig, buttonConfig, dropdownConfig, errorMessgeConfig, mergedOutputFields, options.endpoints?.autocomplete_options);
